...
@onready var hook_connect_audio = $HookConnectAudio
...
var hooked := false :
	set(value):
		...
		if hooked:
			...
			instantiate_shockwave()
			get_tree().get_first_node_in_group("camera").shake_camera()
			hook_connect_audio.pitch_scale = randf_range(.9, 1.1)
			hook_connect_audio.play()
			if gun_pivot.hook_launch_audio.playing:
				gun_pivot.hook_launch_audio.stop()
			...
			if result_vector != Vector2.ONE:
				...
				if !collisions.is_empty():
					...
					if collider_node.is_in_group("level"):
						hooked = true
					elif collider_node.is_in_group("lever"):
						hooked = true
						collider_node.activate()
					else:
						grabbed_node = collider_node
						grabbed_node.grabbed = true
						grabbed_node.collision_polygon_2d.disabled = true
						forward = false
			else:
...
func retract():
	...
	if grabbed_node != null:
		...
		grabbed_node.collision_polygon_2d.set_deferred("disabled", false)
		grabbed_node = null
	retracting = true
	forward = false
	hooked = false
